/* --------------------------------------------------------------------------- *
 * TMOLiu15.cpp: implementation of the TMOLiu15 class.   *
 * --------------------------------------------------------------------------- */
...
TMOLiu15::TMOLiu15() {
    SetName(L"Liu15");
    SetDescription(L"Gradient correlation similarity for efficient contrast preserving decolorization - GcsDecolor2");
...
int TMOLiu15::Transform() {
    ...
    Mat channels2[3];
    split(ims, channels2);
    Rx = channels2[0].colRange(0, channels2[0].cols - 1) - channels2[0].colRange(1, channels2[0].cols);
    Gx = channels2[1].colRange(0, channels2[1].cols - 1) - channels2[1].colRange(1, channels2[1].cols);
    Bx = channels2[2].colRange(0, channels2[2].cols - 1) - channels2[2].colRange(1, channels2[2].cols);
    Ry = channels2[0].rowRange(0, channels2[0].rows - 1) - channels2[0].rowRange(1, channels2[0].rows);
    Gy = channels2[1].rowRange(0, channels2[1].rows - 1) - channels2[1].rowRange(1, channels2[1].rows);
    By = channels2[2].rowRange(0, channels2[2].rows - 1) - channels2[2].rowRange(1, channels2[2].rows);
    ...
    Mat g = W.at<double>(bw, 0) * im_channels[0] + W.at<double>(bw, 1) * im_channels[1] + W.at<double>(bw, 2) * im_channels[2];

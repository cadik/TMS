...
void color2grey(double **fpI, double **fpO, int d_c, int d_w, int d_h)
{
    ...
    for (int ii = 0; ii < d_c; ii++)
        Lab_origin[ii] = new double[d_w * d_h];
    ...
    for (int ii = 0; ii < d_c; ii++)
    { // calculate hilbert transofrmation and reprezentation in complex numbers for computing AM-FM modulation
        ...
        for (int w = 0; w < (d_w * d_h); w += d_h)
        { // calculating partial derivatives from complex pictures
            for (int h = 0; h < d_h; h++)
            {
    ...
    for (int ii = 0; ii < d_c; ii++)
        Lab_modulation[ii] = new double[d_w * d_h];
    double total_L = 0;
    double total_a = 0;
    double total_b = 0;
    for (int w = 0; w < (d_w * d_h); w += d_h)
    ...
    for (int ii = 0; ii < d_c; ii++)
        feature_vec[ii] = new double[d_w * d_h];
    ...
    for (int ii = 0; ii < d_c; ii++)
        alpha_vec[ii] = new double[d_w * d_h];

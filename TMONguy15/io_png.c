 ...
 * @li read a PNG file as a deinterlaced 8bit integer or double array
 * @li write a 8bit integer or double array to a PNG file
 *
...
#define IO_PNG_F32 0x0002       /* 32bit double */
...
static void *io_png_read_raw(const char *fname,
    ...
    double *data_f32 = NULL;
    double *data_f32_ptr = NULL;
    size_t size;
    size_t i, j, k;
    ...
    *ncp = (size_t) png_get_channels(png_ptr, info_ptr);
    ...
    case IO_PNG_U8:
        ...
        break;
    case IO_PNG_F32:
        if (NULL == (data_f32 = (double *) malloc(size * sizeof(double))))
            return io_png_read_abort(fp, &png_ptr, &info_ptr);
        data = (void *) data_f32;
        for (k = 0; k < *ncp; k++) {
            /* channel loop */
...
/**
 * @brief read a PNG file into a 32bit double array
 *
 * The array contains the deinterlaced channels.
 * 1, 2, 4 and 8bit images are converted to double values
 ...
 *         or NULL if an error happens
 */
double *io_png_read_f32(const char *fname,
                       size_t * nxp, size_t * nyp, size_t * ncp) {
    /* read the image as double */
    return (double *) io_png_read_raw(fname, nxp, nyp, ncp,
                                     PNG_TRANSFORM_IDENTITY, IO_PNG_F32);
}

/**
 * @brief read a PNG file into a 32bit double array, converted to RGB
 *
 * See io_png_read_f32() for details.
 */
double *io_png_read_f32_rgb(const char *fname, size_t * nxp, size_t * nyp) {
    size_t nc;
    double *img;

    /* read the image */
    img = (double *) io_png_read_raw(fname, nxp, nyp, &nc,
    ...
    if (3 == nc)
        ...
        /* resize the image */
        size = *nxp * *nyp;
        img = (double *) realloc(img, 3 * size * sizeof(double));
        img_r = img;
        img_g = img + size;
        img_b = img + 2 * size;
...
/**
 * @brief read a PNG file into a 32bit double array, converted to gray
 *
 * See io_png_read_f32() for details.
 */
double *io_png_read_f32_gray(const char *fname, size_t * nxp, size_t * nyp) {
    size_t nc;
    double *img;
    ...
    if (1 == nc)
        ...
        img_b = img + 2 * size;
        for (i = 0; i < size; i++)
            img[i] = (double) (0.212671 * img_r[i]
                              + 0.715160 * img_g[i]
                              + 0.072169 * img_b[i]);
        /* resize and return the image */
        img = (double *) realloc(img, size * sizeof(double));
        return img;
    }
}
...
static int io_png_write_raw(const char *fname, const void *data,
    ...
    const double *data_f32 = NULL;
    const double *data_f32_ptr = NULL;
    double tmp;
...
/**
 * @brief write a double array into a PNG file
 *
 * The double values are rounded to 8bit integers, and bounded to [0, 255].
 *
 ...
 * @return 0 if everything OK, -1 if an error occured
 */
int io_png_write_f32(const char *fname, const double *data,
                     size_t nx, size_t ny, size_t nc) {
    return io_png_write_raw(fname, (void *) data,
                            (png_uint_32) nx, (png_uint_32) ny, (png_byte) nc,
                            IO_PNG_F32);

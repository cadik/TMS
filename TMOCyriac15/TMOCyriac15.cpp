...
cv::Mat TMOCyriac15::createGaussianKernel(int size, double sigma)
{
    ...
    for (int y = 0; y<size; y++)
    {
        ...
        {
            kernel.at<double>(x, y) = std::exp(-((x-center)*(x-center)+(y-center)*(y-center))/(2*sigma*sigma));
        ...
        for (int x = 1; x<I.cols-1; x++) {
            ...
            for (int dy = -1; dy<=1; dy++) {
                for (int dx = -1; dx<=1; dx++) {
                    ...
                    double diff = I.at<double>(y, x)-I.at<double>(y+dy, x+dx);
                    sum += w*sgn(diff)*diff;
                }
            }

            gradient.at<double>(y, x) = -alpha*(I.at<double>(y, x)-localMean.at<double>(y, x))
                +gamma*sum
                -beta*(I.at<double>(y, x)-I0.at<double>(y, x));
        }
    }

...
int TMOGUIWindow::Create()
{
	...
	connect(this, &TMOGUIWindow::imageSelected, pRight->GetMapping(), &TMOGUIToneMapping::windowChanged);
...
void TMOGUIWindow::openFile(QString fileName)
{
	...
	subw->resize(subw->frameSize().expandedTo(pWorkspace->size() / 3)); //TODO check subwindow size
	//connect(subw, &QMdiSubWindow::windowStateChanged, this, &TMOGUIWindow::subWindowStateChanged);

	if (newfile->Open(fileName.toStdString().c_str()))
		delete newfile;
	else
	{
...
void TMOGUIWindow::openFile()
{
	...
	openFile(QFileDialog::getOpenFileName(this, QString(), QString(),
...
void TMOGUIWindow::saveasFile()
{
	...
	if ((iFound = fileName.lastIndexOf(".", -1)) > 0)
	{
		QString sTemp;
		if (iIdx)
			fileName = fileName.left(iFound) + "[" + sTemp.setNum(iIdx) + "].tif";
		else
...
void TMOGUIWindow::closeActivePreviewWindow()
{
	TMOGUIImage *pImage = GetActiveImagePreview();
	QMdiSubWindow *pSubW = getSubwindow(pImage);
	if (pSubW)
	{
		pSubW->close();
		listImage.removeOne(pImage);
	}
}
...
TMOGUIImage *TMOGUIWindow::FindImage(QString name)
{
	...
	{
		if (retval->GetImage() && *retval->imageName == name && !retval->bPreview)
			return retval;
	}
	return nullptr;
}

TMOGUIImage *TMOGUIWindow::FindPreviewImage(QString name)
{

	for (TMOGUIImage *retval : listImage)
	{
		if (retval->GetImage() && *retval->imageName == name && retval->bPreview != false)
			return retval;
	}
	return nullptr;
}

QMdiSubWindow *TMOGUIWindow::getSubwindow(TMOGUIImage *pImage)
{
	if (pImage == nullptr)
		return nullptr;
...
void TMOGUIWindow::activateWindow(const QString &id)
{
	...
	for (QMdiSubWindow *widget : wl)
	{
		if (widget->windowTitle() == id)
		{
			...
			pRight->GetMapping()->windowChanged(pImage);
			break;
		}
	}
}
...
{
	if (!GetActiveImagePreview())
		return;
	if (bLivePreview)
		preview(); // TODO add checkbox for live preview
	return;
}

void TMOGUIWindow::preview()
{
	TMOImage *pSrc = nullptr /*, *pDst = new TMOImage()*/;
	TMOGUIImage *pImage = nullptr;
	QWidget *pWidget;
	...
	{
		pSrc = pImage->GetImage();
		//pDst->New(*pSrc, TMO_NOTSPEC, true);
		pTMO->SetImage(*pSrc);
		//pTMO->SetSource(*pSrc);
		pTransform->SetTMO(pTMO);
	}
...
void TMOGUIWindow::showPreview(TMOGUIImage *pImage)
{
	QList<QMdiSubWindow *> wl = pWorkspace->subWindowList();

	for (QMdiSubWindow *subw : wl)
	{
		if (subw->widget() && !subw->widget()->windowTitle().isEmpty() && subw->widget()->windowTitle() == pImage->windowTitle())
		{
			subw->show();
	...
	QMdiSubWindow *subw = pWorkspace->addSubWindow(pImage);
	subw->resize(subw->frameSize().expandedTo(pWorkspace->size() / 3));
	subw->show();
	fitToWidth();
	pImage->fitHisto();
...
void TMOGUIWindow::undoEdit()
{
	...
	if (!pWorkspace || !(pWorkspace->activeSubWindow()->widget()))
		return;
	sName = pWorkspace->activeSubWindow()->widget()->objectName(); //name()
	pImage = FindImage(sName);
	if (!pImage)
...
void TMOGUIWindow::CreatePreview(TMOGUIImage *pImage)
{
	QString sName = *pImage->imageName;
	TMOGUIImage *newfile = new TMOGUIImage(pProgress, pWorkspace, sName.toStdString().c_str(), true);

	if (newfile)
	{

		if (newfile->NewSmall(pImage))
			delete newfile;
		else
		{
			listImage.append(newfile);
			...
			pInfoTool->SetWindows(pWorkspace);
			pFileTool->SetWindows(pWorkspace);
			pInfo->SetOutput(newfile->pOutput); //TODO check output of preview image
			newfile->deleteDest();
			pProgress->hide();
		}
	}
...
TMOGUIImage *TMOGUIWindow::GetActiveImagePreview()
{
	QMdiSubWindow *pSubWindow = pWorkspace->activeSubWindow();
	if (!pSubWindow)
		return nullptr;
...
void TMOGUIWindow::extractLumCommand()
{
	...
	if (newfile)
	{
		...
		newfile->fitHisto(); //TODO check histo fit
	}
}
...
void TMOGUIWindow::mergeCommand()
{
	...
	s = "";
	if (pImages[0])
		s.append(TMOGUIImage::GetName(*pImages[0]->imageName) + "_");
	if (pImages[1])
		s.append(TMOGUIImage::GetName(*pImages[1]->imageName) + "_");
	if (pImages[2])
		s.append(TMOGUIImage::GetName(*pImages[2]->imageName) + "_");
	s = s.left(s.length());
...
void TMOGUIWindow::MergeComponentsRed(int iImage)
{
	...
	QList<TMOGUIImage *>::const_iterator listImageIter = listImage.begin();
	temp = (*listImageIter); // BUG listImage.next()
	for (int i = iImage; temp && i; i--)
	{
		listImageIter++;
		temp = (*listImageIter);
...
void TMOGUIWindow::operationCommand()
{
	...
	if (pImages[0])
		s.append(TMOGUIImage::GetName(*pImages[0]->imageName) + "_");
	if (pImages[1])
		s.append(TMOGUIImage::GetName(*pImages[1]->imageName));
	s = s.left(s.length());
...
{
	TMOGUIImage *pImage = (TMOGUIImage *)pWorkspace->activeSubWindow()->widget();
	if (!pImage)
		return;
	pImage->fitToScreen(pWorkspace->activeSubWindow()->size()); //TODO check fit to screen
...
{
	TMOGUIImage *pImage = (TMOGUIImage *)pWorkspace->activeSubWindow()->widget(); //GetActiveImage();
	if (!pImage)
		return;
	pImage->zoomIn();
	pTools->SetState();
...
{
	if (advanced == Qt::Checked || advanced == Qt::Unchecked)
		bAdvanced = (advanced == Qt::Checked);
	if (!bAdvanced)
	{
		activateInfoTool(false);
	}
...
void TMOGUIWindow::viewHistogram()
{
	...
	if (!pWindow)
		return;
	//QString sName = pWindow->objectName();
	TMOGUIImage *pImage = (TMOGUIImage *)pWindow; //FindImage(sName);
	if (!pImage)
		return;
	pImage->showtools();

...
void TMOGUIHisto::resizeEvent(QResizeEvent *re)
{
	if (pBackBuffer != nullptr)
		*pBackBuffer = pBackBuffer->scaled(re->size()); // TODO check resize
...
void TMOGUIHisto::compute()
{
	...
	pValues->dMinimum = 0;
	if (length)
		pValues->dMaximum = 0.299 * offset[0];
	...
	if (pValues->dMaximum <= 1 && !bLog)
		pValues->dMaximum = 1;
	if (bLog)
		pValues->dMaximum = log(pValues->dMaximum);
	...
	do
	{
		...
		for (i = 0; i < length; i++)
		{
			...
			{
				if (bLog)
					pValues->dExtreme = exp(l - pValues->dMinimum);
				else
					pValues->dExtreme = l - pValues->dMinimum;

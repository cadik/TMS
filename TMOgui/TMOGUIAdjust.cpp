...
TMOGUIAdjust::TMOGUIAdjust(QWidget *parent, const char *name) : QFrame(parent)
{
	iMode = 0;
	pValues = nullptr;
	bLog = true;
...
void TMOGUIAdjust::setblack(const QString &s)
{
	...
	if (!bUpdate)
		return;
	if (bLog)
		min = exp(pValues->dMinimum);
	else
		min = pValues->dMinimum;
...
void TMOGUIAdjust::setwhite(const QString &s)
{
	...
	if (!bUpdate)
		return;
	if (bLog)
		max = exp(pValues->dMaximum);
	else
		max = pValues->dMaximum;

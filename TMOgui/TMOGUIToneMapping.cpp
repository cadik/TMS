...
TMOGUIToneMapping::TMOGUIToneMapping(QWidget *parent, const char *name /*, Qt::WindowFlags f */) : QWidget(parent)
{
    int i, j, ops = 0;
    QLabel *pLabel;
...
void TMOGUIToneMapping::toggleTechniqueChooser()
{
    ...
    if (!chooserVisible)
        pBackButton->setText("Select TMO");
    else
        pBackButton->setText(name.fromWCharArray(pTMO[iCurTechnique]->GetName()));
}

...
static double Cdisplay01Clinear(double Cdisplay)
{
	...
	Clinear = Cdisplay <= 0.081 ? Cdisplay / 4.5 : pow((Cdisplay + 0.099) / 1.099, 2.2);
 ...
 * @brief Constructor
 */
TMOCadik08::TMOCadik08() : simd{CL_DEVICE_TYPE_DEFAULT},
#ifdef PROFILE
	...
	if (type.GetString() == "cyc" || type.GetString() == "cb" || type.GetString() == "cbloc" || type.GetString() != "hier")
	{
		...
		for (int i = 0; i < ymax; ++i)
		{
			...
			for (int j = 0; j < xmax; ++j)
			{
				int tmp_ind = j + tmp_y;
				// H[i][j + 1] - H[i][j]:
				nablaH[tmp_ind].x = (j + 1) == xmax ? 0. : formulaColoroid(pSourceData, i, j + 1, i, j, xmax);
				// H[i + 1][j] - H[i][j]:
				nablaH[tmp_ind].y = (i + 1) == ymax ? 0. : formulaColoroid(pSourceData, i + 1, j, i, j, xmax);
			}
		}
	...
	else if (type.GetString() == "hier")
	{
		...
		for (int i = 0; i < ymax; ++i)
			for (int j = 0; j < xmax; ++j)
			{
				...
				// H[i][j + 1] - H[i][j]:
				nablaH[z].x = (j + 1) == xmax ? 0. : formulaColoroid(pSourceData, i, j + 1, i, j, xmax);
				// H[i + 1][j] - H[i][j]:
				nablaH[z].y = (i + 1) == ymax ? 0. : formulaColoroid(pSourceData, i + 1, j, i, j, xmax);
			}
...
void TMOCadik08::integrate2x(TMOImage &g, TMOImage &o) const
{
	...
	const cl::buffer grad{simd.create_buffer(CL_MEM_READ_WRITE,
		...
		out{simd.create_buffer(CL_MEM_READ_WRITE |
							   ...
							   o.GetData())};
	cl::event status{step.write_buffer(grad, 0, rows * cols * 3 * sizeof(double),
									   g.GetData())};

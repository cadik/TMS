  ...
  for (int xyz = 0, z = 0; z < od; z++)
    for (int y = 0; y < oh; y++)
      for (int x = 0; x < ow; x++, xyz++)
      {
        ...
        RC(LEE, v) = cap_lee[xyz];
        if (cap_lee[xyz] != 0)
        {
          DESAT(LEE, v);
        }
        RC(GEE, v) = cap_gee[xyz];
        if (cap_gee[xyz] != 0)
        {
          DESAT(GEE, v);
        }
        RC(ELE, v) = cap_ele[xyz];
        if (cap_ele[xyz] != 0)
        {
          DESAT(ELE, v);
        }
        RC(EGE, v) = cap_ege[xyz];
        if (cap_ege[xyz] != 0)
        {
          DESAT(EGE, v);
        }
        RC(EEL, v) = cap_eel[xyz];
        if (cap_eel[xyz] != 0)
        {
          DESAT(EEL, v);
        }
        RC(EEG, v) = cap_eeg[xyz];
        if (cap_eeg[xyz] != 0)
        {
          DESAT(EEG, v);
        }
        RC(LLE, v) = cap_lle[xyz];
        if (cap_lle[xyz] != 0)
        {
          DESAT(LLE, v);
        }
        RC(GLE, v) = cap_gle[xyz];
        if (cap_gle[xyz] != 0)
        {
          DESAT(GLE, v);
        }
        RC(LGE, v) = cap_lge[xyz];
        if (cap_lge[xyz] != 0)
        {
          DESAT(LGE, v);
        }
        RC(GGE, v) = cap_gge[xyz];
        if (cap_gge[xyz] != 0)
        {
          DESAT(GGE, v);
        }
        RC(ELL, v) = cap_ell[xyz];
        if (cap_ell[xyz] != 0)
        {
          DESAT(ELL, v);
        }
        RC(EGL, v) = cap_egl[xyz];
        if (cap_egl[xyz] != 0)
        {
          DESAT(EGL, v);
        }
        RC(ELG, v) = cap_elg[xyz];
        if (cap_elg[xyz] != 0)
        {
          DESAT(ELG, v);
        }
        RC(EGG, v) = cap_egg[xyz];
        if (cap_egg[xyz] != 0)
        {
          DESAT(EGG, v);
        }
        RC(LEL, v) = cap_lel[xyz];
        if (cap_lel[xyz] != 0)
        {
          DESAT(LEL, v);
        }
        RC(LEG, v) = cap_leg[xyz];
        if (cap_leg[xyz] != 0)
        {
          DESAT(LEG, v);
        }
        RC(GEL, v) = cap_gel[xyz];
        if (cap_gel[xyz] != 0)
        {
          DESAT(GEL, v);
        }
        RC(GEG, v) = cap_geg[xyz];
        if (cap_geg[xyz] != 0)
        {
          DESAT(GEG, v);
        }
        RC(LLL, v) = cap_lll[xyz];
        if (cap_lll[xyz] != 0)
        {
          DESAT(LLL, v);
        }
        RC(GLL, v) = cap_gll[xyz];
        if (cap_gll[xyz] != 0)
        {
          DESAT(GLL, v);
        }
        RC(LGL, v) = cap_lgl[xyz];
        if (cap_lgl[xyz] != 0)
        {
          DESAT(LGL, v);
        }
        RC(GGL, v) = cap_ggl[xyz];
        if (cap_ggl[xyz] != 0)
        {
          DESAT(GGL, v);
        }
        RC(LLG, v) = cap_llg[xyz];
        if (cap_llg[xyz] != 0)
        {
          DESAT(LLG, v);
        }
        RC(GLG, v) = cap_glg[xyz];
        if (cap_glg[xyz] != 0)
        {
          DESAT(GLG, v);
        }
        RC(LGG, v) = cap_lgg[xyz];
        if (cap_lgg[xyz] != 0)
        {
          DESAT(LGG, v);
        }
        RC(GGG, v) = cap_ggg[xyz];
        if (cap_ggg[xyz] != 0)
        {
          DESAT(GGG, v);
        }
...
template <typename type_tcap, typename type_ncap, typename type_flow>
GridGraph_3D_26C<type_tcap, type_ncap, type_flow>::GridGraph_3D_26C(int w, int h, int d) : ow(w),
                                                                                           oh(h),
                                                                                           od(d),
                                                                                           ...
                                                                                           mem_pool(NULL)
{
  size_t mem_pool_size = (W * H * D * sizeof(unsigned char) + 64 +    // label

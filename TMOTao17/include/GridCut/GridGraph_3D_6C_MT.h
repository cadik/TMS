                                                                ...
                                                                const int ZOFS)
{
  ...
  while (!((orphans1_back == 0) && (orphans2_front == 0)))
  {
    ...
    {
      const int N = N_ID[i];
      if (LABEL(N) == tree_p && PARENT(N) == SISTER(i))
      {
        PARENT(N) = NONE;
        ORPHANS2_PUSH_BACK(N, orphans2_front, orphans2_back);
      }
    }
    ...
    for (int y = y_min; y < y_max; y++)
      for (int x = x_min; x < x_max; x++)
      {
        ...
        for (int arc = 0; arc < 6; arc++)
        {
          ...
          {
            if (NONSAT(SISTER(arc), N) && LABEL(N) == LABEL_F)
            {
              ACTIVE_PUSH_BACK(v, active_front, active_back);
              goto next_node;
            }
                     ...
                     const int ZOFS)
{
  if (segment.type == SEGMENT_X)
  {
    for (int i = 0; i < segment.length; i++)
    {
      ...
      if (LABEL(v) == LABEL_S && NONSAT(GEE, v) && LABEL(N_GEE(v)) != LABEL_S)
        ACTIVE_PUSH_BACK(v, active_front, active_back);
      if (LABEL(v) == LABEL_T && NONSAT(LEE, N_GEE(v)) && LABEL(N_GEE(v)) != LABEL_T)
        ACTIVE_PUSH_BACK(v, active_front, active_back);

      if (LABEL(N_GEE(v)) == LABEL_S && NONSAT(LEE, N_GEE(v)) && LABEL(v) != LABEL_S)
        ACTIVE_PUSH_BACK(N_GEE(v), active_front, active_back);
      if (LABEL(N_GEE(v)) == LABEL_T && NONSAT(GEE, v) && LABEL(v) != LABEL_T)
        ACTIVE_PUSH_BACK(N_GEE(v), active_front, active_back);
    }
  }
  else if (segment.type == SEGMENT_Y)
  {
    for (int i = 0; i < segment.length; i++)
    {
      ...
      if (LABEL(v) == LABEL_S && NONSAT(EGE, v) && LABEL(N_EGE(v)) != LABEL_S)
        ACTIVE_PUSH_BACK(v, active_front, active_back);
      if (LABEL(v) == LABEL_T && NONSAT(ELE, N_EGE(v)) && LABEL(N_EGE(v)) != LABEL_T)
        ACTIVE_PUSH_BACK(v, active_front, active_back);

      if (LABEL(N_EGE(v)) == LABEL_S && NONSAT(ELE, N_EGE(v)) && LABEL(v) != LABEL_S)
        ACTIVE_PUSH_BACK(N_EGE(v), active_front, active_back);
      if (LABEL(N_EGE(v)) == LABEL_T && NONSAT(EGE, v) && LABEL(v) != LABEL_T)
        ACTIVE_PUSH_BACK(N_EGE(v), active_front, active_back);
    }
  }
  else if (segment.type == SEGMENT_Z)
  {
    for (int i = 0; i < segment.length; i++)
    {
      ...
      if (LABEL(v) == LABEL_S && NONSAT(EEG, v) && LABEL(N_EEG(v)) != LABEL_S)
        ACTIVE_PUSH_BACK(v, active_front, active_back);
      if (LABEL(v) == LABEL_T && NONSAT(EEL, N_EEG(v)) && LABEL(N_EEG(v)) != LABEL_T)
        ACTIVE_PUSH_BACK(v, active_front, active_back);

      if (LABEL(N_EEG(v)) == LABEL_S && NONSAT(EEL, N_EEG(v)) && LABEL(v) != LABEL_S)
        ACTIVE_PUSH_BACK(N_EEG(v), active_front, active_back);
      if (LABEL(N_EEG(v)) == LABEL_T && NONSAT(EEG, v) && LABEL(v) != LABEL_T)
        ACTIVE_PUSH_BACK(N_EEG(v), active_front, active_back);
    }
  }
}
  ...
  for (int z = 1; z < _d + 1; z++)
    for (int y = 1; y < _h + 1; y++)
      for (int x = 1; x < _w + 1; x++)
      {
        ...
        rc_lee[v] = cap_lee[xyz];
        if (cap_lee[xyz] != 0)
        {
          DESAT(LEE, v);
        }
        rc_gee[v] = cap_gee[xyz];
        if (cap_gee[xyz] != 0)
        {
          DESAT(GEE, v);
        }
        rc_ele[v] = cap_ele[xyz];
        if (cap_ele[xyz] != 0)
        {
          DESAT(ELE, v);
        }
        rc_ege[v] = cap_ege[xyz];
        if (cap_ege[xyz] != 0)
        {
          DESAT(EGE, v);
        }
        rc_eel[v] = cap_eel[xyz];
        if (cap_eel[xyz] != 0)
        {
          DESAT(EEL, v);
        }
        rc_eeg[v] = cap_eeg[xyz];
        if (cap_eeg[xyz] != 0)
        {
          DESAT(EEG, v);
        }
...
template <typename type_tcap, typename type_ncap, typename type_flow>
GridGraph_3D_6C_MT<type_tcap, type_ncap, type_flow>::GridGraph_3D_6C_MT(int w, int h, int d, int num_threads, int block_size) : _w(w),
                                                                                                                                _h(h),
                                                                                                                                _d(d),

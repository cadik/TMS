...
private:
  ...
  inline void Q_PUSH_BACK1(int v)
  {
    if (!QN[v])
    {
      QN[QB] = v;
  ...
  inline void Q_PUSH_BACK2(int v)
  {
    if (!QN[v])
    {
      QN[QB] = v;
                                                                       ...
                                                                       const type_arg_ncap *cap_eg)
{
  ...
  for (int v = 0; v < W * H; v++)
  {
    ...
    for (int arc = 0; arc < 4; arc++)
    {
      ...
      {
        if (NONSAT_SISTER(arc, N) && LABEL(N) == LABEL_F)
        {
          Q_PUSH_BACK1(v);
          goto next_node;
        }
...
template <typename type_tcap, typename type_ncap, typename type_flow>
GridGraph_2D_4C<type_tcap, type_ncap, type_flow>::GridGraph_2D_4C(int w, int h) : ow(w),
                                                                                  oh(h),

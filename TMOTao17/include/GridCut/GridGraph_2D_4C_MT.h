...
extern "C" __declspec(dllimport) void *__stdcall CreateThread(_SECURITY_ATTRIBUTES *,
#if defined(_WIN64)
                                                              unsigned __int64,
#else
                                                              unsigned long,
...
private:
  ...
  {
    orphans1_prev[v] = orphans1_back;
    orphans1_back = v;
  }

  inline int ORPHANS1_POP_BACK(int &orphans1_back)
  {
    const int tmp = orphans1_back;
    orphans1_back = orphans1_prev[tmp];
    orphans1_prev[tmp] = 0;
    return tmp;
  }
  ...
  {
    orphans2_next[o2_back] = V;
    o2_front = o2_front ? o2_front : V;
    o2_back = V;
  }

  inline int ORPHANS2_POP_FRONT(int &o2_front, int &o2_back)
  {
    const int T = o2_front;
    o2_front = orphans2_next[T];
    orphans2_next[T] = 0;
    if (!o2_front)
    {
      o2_back = 0;
  ...
  {
    free_nodes_prev[v] = free_nodes_back;
    free_nodes_back = v;
  }

  inline int FREE_NODES_POP_BACK(int &free_nodes_back)
  {
    const int tmp = free_nodes_back;
    free_nodes_back = free_nodes_prev[tmp];
    free_nodes_prev[tmp] = 0;
    return tmp;
  }
...
#define THREAD_FUNC_WRAPPER_DECL static void *thread_func_wrapper(void *arg)
  ...
  template <typename T>
  inline type_ncap mincap(type_ncap a, T b) const
  {
    return a < b ? a : b;
  }
...
#define ATOMIC_INCREMENT(X) (__sync_fetch_and_add(X, 1))
  ...
  template <typename T>
  inline const T &minimum(const T &a, const T &b) const
  {
    return a < b ? a : b;
  }

  template <typename T>
  inline const T &maximum(const T &a, const T &b) const
  {
    return a > b ? a : b;
  }
    ...
    if (tree_p == LABEL_S)
    {
      for (int n = 0; n < 4; n++)
      {
        ...
        if (NONSAT(n, P))
        {
          ...
          {
            LABEL(N) = LABEL_S;
            ACTIVE_PUSH_BACK(N, active_front, active_back);
            PARENT(N) = SISTER(n);
            PARENT_ID(N) = P;
          }
        }
                                                                ...
                                                                const int YOFS)
{
  ...
  while (!((orphans1_back == 0) && (orphans2_front == 0)))
  {
    ...
    if (tree_p == LABEL_S)
    {
      if (NONSAT(GE, L) && LABEL(L) == LABEL_S && find_origin(L, TIME) == TERMINAL)
      {
        timestamp[V] = TIME;
        PARENT(V) = LE;
        PARENT_ID(V) = L;
      ...
      if (NONSAT(LE, R) && LABEL(R) == LABEL_S && find_origin(R, TIME) == TERMINAL)
      {
        timestamp[V] = TIME;
        PARENT(V) = GE;
        PARENT_ID(V) = R;
      ...
      if (NONSAT(EG, T) && LABEL(T) == LABEL_S && find_origin(T, TIME) == TERMINAL)
      {
        timestamp[V] = TIME;
        PARENT(V) = EL;
        PARENT_ID(V) = T;
      ...
      if (NONSAT(EL, B) && LABEL(B) == LABEL_S && find_origin(B, TIME) == TERMINAL)
      {
        timestamp[V] = TIME;
        PARENT(V) = EG;
        PARENT_ID(V) = B;
    ...
    {
      if (NONSAT(LE, V) && LABEL(L) == LABEL_T && find_origin(L, TIME) == TERMINAL)
      {
        timestamp[V] = TIME;
        PARENT(V) = LE;
        PARENT_ID(V) = L;
      ...
      if (NONSAT(GE, V) && LABEL(R) == LABEL_T && find_origin(R, TIME) == TERMINAL)
      {
        timestamp[V] = TIME;
        PARENT(V) = GE;
        PARENT_ID(V) = R;
      ...
      if (NONSAT(EL, V) && LABEL(T) == LABEL_T && find_origin(T, TIME) == TERMINAL)
      {
        timestamp[V] = TIME;
        PARENT(V) = EL;
        PARENT_ID(V) = T;
      ...
      if (NONSAT(EG, V) && LABEL(B) == LABEL_T && find_origin(B, TIME) == TERMINAL)
      {
        timestamp[V] = TIME;
        PARENT(V) = EG;
        PARENT_ID(V) = B;
    ...
    if (LABEL(L) == tree_p && PARENT(L) == GE)
    {
      PARENT(L) = NONE;
      ORPHANS2_PUSH_BACK(L, orphans2_front, orphans2_back);
    }
    if (LABEL(R) == tree_p && PARENT(R) == LE)
    {
      PARENT(R) = NONE;
      ORPHANS2_PUSH_BACK(R, orphans2_front, orphans2_back);
    }
    if (LABEL(T) == tree_p && PARENT(T) == EG)
    {
      PARENT(T) = NONE;
      ORPHANS2_PUSH_BACK(T, orphans2_front, orphans2_back);
    }
    if (LABEL(B) == tree_p && PARENT(B) == EL)
    {
      PARENT(B) = NONE;
      ORPHANS2_PUSH_BACK(B, orphans2_front, orphans2_back);
    }
  ...
  while (!(free_nodes_back == 0))
  {
    ...
    if (NONSAT(GE, L) && LABEL(L) == LABEL_S)
    {
      ACTIVE_PUSH_BACK(L, active_front, active_back);
    }
    if (NONSAT(LE, R) && LABEL(R) == LABEL_S)
    {
      ACTIVE_PUSH_BACK(R, active_front, active_back);
    }
    if (NONSAT(EG, T) && LABEL(T) == LABEL_S)
    {
      ACTIVE_PUSH_BACK(T, active_front, active_back);
    }
    if (NONSAT(EL, B) && LABEL(B) == LABEL_S)
    {
      ACTIVE_PUSH_BACK(B, active_front, active_back);
    }

    if (NONSAT(LE, V) && LABEL(L) == LABEL_T)
    {
      ACTIVE_PUSH_BACK(L, active_front, active_back);
    }
    if (NONSAT(GE, V) && LABEL(R) == LABEL_T)
    {
      ACTIVE_PUSH_BACK(R, active_front, active_back);
    }
    if (NONSAT(EL, V) && LABEL(T) == LABEL_T)
    {
      ACTIVE_PUSH_BACK(T, active_front, active_back);
    }
    if (NONSAT(EG, V) && LABEL(B) == LABEL_T)
    {
      ACTIVE_PUSH_BACK(B, active_front, active_back);
    }
  }
               ...
               const int YOFS)
{
  ...
  for (int y = y_min; y < y_max; y++)
    for (int x = x_min; x < x_max; x++)
    {
      ...
      if (lv == LABEL_S)
      {
        if (NONSAT(LE, v) && LABEL(L) != lv)
        {
          ACTIVE_PUSH_BACK(v, active_front, active_back);
          continue;
        }
        if (NONSAT(GE, v) && LABEL(R) != lv)
        {
          ACTIVE_PUSH_BACK(v, active_front, active_back);
          continue;
        }
        if (NONSAT(EL, v) && LABEL(T) != lv)
        {
          ACTIVE_PUSH_BACK(v, active_front, active_back);
          continue;
        }
        if (NONSAT(EG, v) && LABEL(B) != lv)
        {
          ACTIVE_PUSH_BACK(v, active_front, active_back);
          continue;
        }
      ...
      {
        if (NONSAT(GE, L) && LABEL(L) == LABEL_F)
        {
          ACTIVE_PUSH_BACK(v, active_front, active_back);
          continue;
        }
        if (NONSAT(LE, R) && LABEL(R) == LABEL_F)
        {
          ACTIVE_PUSH_BACK(v, active_front, active_back);
          continue;
        }
        if (NONSAT(EG, T) && LABEL(T) == LABEL_F)
        {
          ACTIVE_PUSH_BACK(v, active_front, active_back);
          continue;
        }
        if (NONSAT(EL, B) && LABEL(B) == LABEL_F)
        {
          ACTIVE_PUSH_BACK(v, active_front, active_back);
          continue;
        }
                     ...
                     const int YOFS)
{
  if (segment.type == SEGMENT_HORIZONTAL)
  {
    for (int i = 0; i < segment.length; i++)
    {
      ...
      if (LABEL(v) == LABEL_S && NONSAT(EG, v) && LABEL(N_EG(v)) != LABEL_S)
        ACTIVE_PUSH_BACK(v, active_front, active_back);
      if (LABEL(v) == LABEL_T && NONSAT(EL, N_EG(v)) && LABEL(N_EG(v)) != LABEL_T)
        ACTIVE_PUSH_BACK(v, active_front, active_back);

      if (LABEL(N_EG(v)) == LABEL_S && NONSAT(EL, N_EG(v)) && LABEL(v) != LABEL_S)
        ACTIVE_PUSH_BACK(N_EG(v), active_front, active_back);
      if (LABEL(N_EG(v)) == LABEL_T && NONSAT(EG, v) && LABEL(v) != LABEL_T)
        ACTIVE_PUSH_BACK(N_EG(v), active_front, active_back);
    }
  }
  else if (segment.type == SEGMENT_VERTICAL)
  {
    for (int i = 0; i < segment.length; i++)
    {
      ...
      if (LABEL(v) == LABEL_S && NONSAT(GE, v) && LABEL(N_GE(v)) != LABEL_S)
        ACTIVE_PUSH_BACK(v, active_front, active_back);
      if (LABEL(v) == LABEL_T && NONSAT(LE, N_GE(v)) && LABEL(N_GE(v)) != LABEL_T)
        ACTIVE_PUSH_BACK(v, active_front, active_back);

      if (LABEL(N_GE(v)) == LABEL_S && NONSAT(LE, N_GE(v)) && LABEL(v) != LABEL_S)
        ACTIVE_PUSH_BACK(N_GE(v), active_front, active_back);
      if (LABEL(N_GE(v)) == LABEL_T && NONSAT(GE, v) && LABEL(v) != LABEL_T)
        ACTIVE_PUSH_BACK(N_GE(v), active_front, active_back);
    }
  }
}
...
void GridGraph_2D_4C_MT<type_tcap, type_ncap, type_flow>::compute_maxflow()
{
  ...
  for (int i = 0; i < NUM_THREADS; i++)
    MAXFLOW_TOTAL += threads[i]->MAXFLOW;
...
template <typename type_tcap, typename type_ncap, typename type_flow>
GridGraph_2D_4C_MT<type_tcap, type_ncap, type_flow>::GridGraph_2D_4C_MT(int w, int h, int num_threads, int block_size) : _w(w),
                                                                                                                         _h(h),

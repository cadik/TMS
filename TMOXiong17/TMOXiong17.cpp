				...
				if (((r + g + b) <= order) && ((r + g + b) > 0))
				{
					...
					for (unsigned int row = 0; row < IMAGE_HEIGHT; ++row)
					{
						for (unsigned int col = 0; col < IMAGE_WIDTH; ++col)
						{
							...
							curIm.at<float>(col + IMAGE_WIDTH * row, 0) = pow((*(image + (col + IMAGE_WIDTH * row) * 3 + RED_CHOSEN)), r) * pow((*(image + (col + IMAGE_WIDTH * row) * 3 + GREEN_CHOSEN)), g) * pow((*(image + (col + IMAGE_WIDTH * row) * 3 + BLUE_CHOSEN)), b);
						}
					}
